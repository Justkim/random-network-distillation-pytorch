...
class MarioEnv(Process):
    ...
    def make_env(self, env_idx):
        ...
        levelList = ['SuperMarioBros-v0', 'SuperMarioBros-2-1-v0',
                     'SuperMarioBros-3-1-v0', 'SuperMarioBros-4-1-v0',
                     'SuperMarioBros-5-1-v0', 'SuperMarioBros-6-1-v0',
                     'SuperMarioBros-7-1-v0', 'SuperMarioBros-8-1-v0']
    ...
    def run(self):
        while True:
            ...
            for i in range(0, self.action_re):
                ...
                if done:
                    print("env: " + str(self.env_id) + " episode: " + str(
                        self.ep_num) + " progress; " + str(
                        self.progress_reward) + " max_x: " + str(
                        info['x_pos']))
                    self.ep_num += 1
                    self.progress_reward = 0
                    obs = self.env.reset()
    ...
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255,
                                                shape=(
                                                      ...
                                                      1),
                                                dtype=np.uint8)
        self.frame_deque = deque([np.zeros((self.height, self.width)),
